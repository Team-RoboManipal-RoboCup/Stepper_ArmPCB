...
typedef enum {
    ...
    CMD_GET_POS,
    CMD_GET_POS_ANGLE,
	CMD_STEPPER_MODE_CLOSED_LOOP,
	CMD_STEPPER_MODE_OPEN_LOOP,
	CMD_OPEN_SPEED
} usb_cmd_t;
...
//for changing mode of stepper, default is closed loop
typedef enum {
    STEPPER_MODE_CLOSED_LOOP,
    STEPPER_MODE_OPEN_LOOP
} stepper_mode_t;

volatile stepper_mode_t stepper_mode = STEPPER_MODE_CLOSED_LOOP;

//open loop variables
volatile float open_loop_steps_per_sec = 0.0f;
volatile int   open_loop_dir = +1;
...
usb_cmd_t parse_usb_command(uint8_t *data, uint32_t len)
{
    ...
    if (!strcmp(cmd, "OPEN_LOOP")) return CMD_STEPPER_MODE_OPEN_LOOP;
    if (!strcmp(cmd, "CLOSED_LOOP")) return CMD_STEPPER_MODE_CLOSED_LOOP;
    if (!strcmp(cmd, "OPEN_SPEED")) return CMD_OPEN_SPEED; //command is OPEN_SPEED <stepsPerSec> <direction>
...
void USB_Command_Handle(uint8_t *data, uint32_t len)
{
    ...
    case CMD_STEPPER_MODE_CLOSED_LOOP:
    {
        stepper_mode = STEPPER_MODE_CLOSED_LOOP;

        CDC_Transmit_FS(
            (uint8_t *)"MODE CHANGED TO CLOSED\r\n",
            strlen("MODE CHANGED TO CLOSED\r\n")
        );

        break;
    }

    case CMD_STEPPER_MODE_OPEN_LOOP:
    {
        stepper_mode = STEPPER_MODE_OPEN_LOOP;

        CDC_Transmit_FS(
            (uint8_t *)"MODE CHANGED TO OPEN\r\n",
            strlen("MODE CHANGED TO OPEN\r\n")
        );
    ...
    case CMD_OPEN_SPEED:
    {
        const float MAX_STEPS_PER_SEC = 20000.0f;  // hard limit
        ...
        float speed = atof(p);
        if (speed <= 0.0f || speed > MAX_STEPS_PER_SEC) {
            usb_err("ERR VAL\r\n");
            break;
        }
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (AS5600_ReadAngle_deg(&hi2c1, &angle) == HAL_OK)
	  {
	      ...
	      if (stepper_mode == STEPPER_MODE_CLOSED_LOOP) {
	          stepper_cfg.target_angle_deg = (float)received_angle_deg;
	  ...
	  if (stepper_mode == STEPPER_MODE_OPEN_LOOP) {
	      Stepper_Actuate(
	          &stepper_cfg,
	          &stepper_state,
	          open_loop_steps_per_sec,
	          open_loop_dir
	      );
	  }
